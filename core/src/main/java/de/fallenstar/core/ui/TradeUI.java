 ...
 *
 * **WICHTIG:** TradeUI nutzt TradingEntity.getTradeSets() das List<?> zurückgibt.
 * Module müssen den Cast zu ihrem TradeSet-Typ selbst durchführen.
 *
 * @author FallenStar
 * @version 1.0
 */
public class TradeUI {
    ...
    public static void openTradeUI(Player player, TradingEntity trader) {
        ...
        // getTradeSets() gibt List<?> zurück - wir konvertieren zu Recipes ohne Cast
        ...
        // Wir nutzen duck-typing: Jedes Objekt mit createRecipe() funktioniert
        List<MerchantRecipe> recipes = tradeSets.stream()
                .map(tradeSet -> {
                    try {
                        // Reflection: Rufe createRecipe() auf dem Objekt auf
                        return (MerchantRecipe) tradeSet.getClass()
                                .getMethod("createRecipe")
                                .invoke(tradeSet);
                    } catch (Exception e) {
                        logger.warning("Failed to create recipe from TradeSet: " + e.getMessage());
                        return null;
                    }
                })
        ...
        if (recipes.isEmpty()) {
            player.sendMessage("§cFehler beim Laden der Handels-Angebote.");
            logger.severe("No valid recipes created from TradeSets!");
            return;
        }
